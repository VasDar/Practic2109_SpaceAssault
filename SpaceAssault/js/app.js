...
function initMegaliths() {
    ...
    for (var i = 0; i < countmegaliths; i++) {
        ...
        var megalithPosx = randomInteger(60, 452);

        var megalithPosy = randomInteger(60, 420);
...
var manna = [];
var mannacount = 0;
var mannacountEl = document.getElementById('mannacount');
...
function checkIntersection(dx, dy = 0) {
...
function handleInput(dt) {
    if (input.isDown('DOWN') || input.isDown('s')) {
        if (!(checkIntersection(0, playerSpeed * dt))) {
    ...
    if (input.isDown('LEFT') || input.isDown('a')) {
        if (!(checkIntersection(dx = -playerSpeed * dt))) {
